...
create table public.projects(id uuid primary key default gen_random_uuid(),number text unique not null,quote_id uuid references public.quotes(id),client_id uuid not null references public.clients(id),status public.project_status not null default 'PLANEAMENTO',area_m2 numeric(12,2) not null default 0,planned_cost numeric(12,2) not null default 0,actual_cost numeric(12,2) not null default 0,sale_price numeric(12,2) not null default 0,start_date date,end_date date,created_at timestamptz not null default now());
...
create table public.payments(id uuid primary key default gen_random_uuid(),client_id uuid references public.clients(id),project_id uuid references public.projects(id),amount numeric(12,2) not null default 0,payment_date date not null default current_date,method text,notes text);
